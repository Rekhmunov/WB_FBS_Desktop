...
class PrintSupplyStickersOnDemandTests(unittest.TestCase):
    ...
    def test_print_resumes_disk_and_fetches_only_missing(
        self, product_svc, _existing, fetch_map, fetch_cards
    ):
        from app.services import print_docs
        ...
        with patch(
            ...
            path = print_docs.print_supply_stickers(
                MagicMock(),
                orders_svc,
                1,
                "api-key",
        ...
        self.assertTrue(str(path).endswith(".html") or path.name.startswith("feedpilot_stickers"))
